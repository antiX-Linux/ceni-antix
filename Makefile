# This Makefile is for the Ceni::Backend extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Ceni/Backend.pm]
#     AUTHOR => [q[Kel Modderman <kelmo@otaku42.de>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     EXE_FILES => [q[bin/Ceni], q[bin/nicinfo]]
#     NAME => q[Ceni::Backend]
#     PREREQ_PM => {  }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Ceni/Backend.pm]
...
MAN1PODS = bin/Ceni \
	bin/nicinfo
MAN3PODS = lib/Ceni/Backend.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Ceni
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Ceni
...
TO_INST_PM = lib/Ceni/Backend.pm

PM_TO_BLIB = lib/Ceni/Backend.pm \
	blib/lib/Ceni/Backend.pm
...
DISTNAME = Ceni-Backend
DISTVNAME = Ceni-Backend-1
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN1EXT) --perm_rw=$(PERM_RW) -u \
	  bin/Ceni $(INST_MAN1DIR)/Ceni.$(MAN1EXT) \
	  bin/nicinfo $(INST_MAN1DIR)/nicinfo.$(MAN1EXT) 
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/Ceni/Backend.pm $(INST_MAN3DIR)/Ceni::Backend.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Perl extension for Ceni'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Kel Modderman <kelmo@otaku42.de>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Ceni-Backend' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Perl extension for Ceni",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Kel Modderman <kelmo@otaku42.de>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Ceni-Backend",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl extension for Ceni</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Kel Modderman &lt;kelmo@otaku42.de&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Ceni/Backend.pm blib/lib/Ceni/Backend.pm 
